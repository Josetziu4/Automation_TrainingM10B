...
namespace Automation_TrainingM10B.Base_Files
{
    class QueryUtils
    {
        ...
        {
            T output = (T)Activator.CreateInstance(typeof(T));//Poner entre parentesis al principio un tipo de dato es un "CAST"... lo convierte
            ...
            {
                if (!dbReader.HasRows)//Si el query no tiene datos..
                {
                    return output;
                }
